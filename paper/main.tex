...
and FLOPs (52.5M parameters, 100M tokens, 3--5 seeds), SwiGLU, LL1, and dense
...
\item \textbf{End-to-end:} at 52.5M parameters / 100M tokens (3--5 seeds),
  LL1($L\in\{2,4,8\}$), SwiGLU, and Tucker tie on loss (every $L\ge2$ nominally below
  the SwiGLU mean; Welch $|t|\le1.4$); LL1 is 5--7\% faster than SwiGLU and $2.1\times$
...
only in FFN (Table~\ref{tab:configs}); 5 seeds for SwiGLU, 3 each for
LL1($L\in\{2,4,8\}$) and Tucker, 1 each for LL1($L\in\{1,16\}$). Tucker uses its
best-known (diagonal warm-start) initialization. Throughput is measured separately on
an idle A40.

\textbf{Result.} Final validation loss (mean $\pm$ sample std; Figure~\ref{fig:lm}):
SwiGLU $4.7509 \pm 0.0157$ ($n{=}5$); LL1 $4.7409 \pm 0.0044$ at $L{=}2$,
$4.7472 \pm 0.0050$ at $L{=}4$, $4.7427 \pm 0.0025$ at $L{=}8$ ($n{=}3$ each);
Tucker $4.7626 \pm 0.0087$ ($n{=}3$). Every LL1 point with $L \ge 2$ is nominally
below the SwiGLU mean, but no LL1--SwiGLU comparison is significant (Welch
$|t| \le 1.4$; pooled $L\in\{2,4,8\}$ vs.\ SwiGLU: $t = 1.0$): \emph{a statistical
tie}. LL1 vs.\ Tucker does separate ($t = 3.8$ at $L{=}8$). Single-seed
endpoints: $L{=}1$: $4.765$ (inside the SwiGLU seed range, confirming the
$L{=}1\equiv$SwiGLU control end-to-end); $L{=}16$: $4.750$. A secondary observation
we do not over-read: LL1's seed variance is $3$--$6\times$ smaller than SwiGLU's at
every $L$. Training throughput at matched FLOPs
...
inherited from SwiGLU tuning; the $L$-sweep endpoints ($L{=}1, 16$) are single-seed.
...
tokens). Left: final validation loss across the LL1 block-rank sweep (error bars =
std over 3 seeds at $L\in\{2,4,8\}$; $L\in\{1,16\}$ single-seed), with SwiGLU
($n{=}5$, blue) and dense Tucker ($n{=}3$, red) mean$\,\pm\,$std bands. Every
$L \ge 2$ sits below the SwiGLU mean but within its band: a statistical tie. Right:
validation-loss curves (mean $\pm$ std) --- the three families are indistinguishable
through training.}
\label{fig:lm}
\end{figure}
