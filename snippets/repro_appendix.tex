\paragraph{Tucker LM and SwiGLU LM training.}
...
(vocabulary $50{,}257$). Context length $T{=}1024$,
batch size $B{=}24$ sequences
($24$K tokens per optimizer step). Optimizer:
...
of peak; peak LR $0.0003$. Precision: bf16 autocast for the
forward/backward, fp32 master parameters and optimizer state.
Hardware: single NVIDIA A40 (46~GB).
Total: $\approx6000$ optimizer steps,
$147$M training tokens.
...
split ($32$K tokens), used to estimate the per-channel mean
...
$d{=}64$. Per teacher, $P, Q, R \sim \mathcal{N}(0, 1/d)$;
...
train/val sizes $50000$/$5000$.
Adam, lr $0.003$, batch $512$,
$8000$ steps with cosine schedule
($1\%$ floor, no warmup). $8$ seeds per
...
Teacher: layer $\ell{=}4$ of a trained Tucker
...
$80000$/$8000$ tokens.
Adam, lr $0.001$, batch $512$,
$8000$ steps with cosine schedule
($1\%$ floor, $5\%$ warmup) and gradient clipping $1.0$.
$3$ seeds per budget; error bars are
